// Updated to call BE endpoint: /classes

document.addEventListener('DOMContentLoaded', async function() {
  if (!requireAuthOrRedirect()) return;
  try {
    const raw = await apiFetch('/classes');
    const items = (Array.isArray(raw) ? raw : (raw?.data || []));
    const mapped = items.map((x) => {
      
      // Map BE Class -> FE card fields (keep FE UI stable)
      const status = x.status || 'PENDING';
      const progress = x.sessions ? `${x.sessions.length} buổi` : (x.status === 'CLOSED' ? 'Đã kết thúc' : '—');
      return {
        id: x.id,
        subject: `${x.subject}${x.classID ? ' - ' + x.classID : ''}`,
        level: x.level || '',
        students: '—',
        schedule: x.schedule || '',
        fee: x.fee || (x.amount ? String(x.amount) : ''),
        progress,
        role: 'STAFF'
      };

    });
    renderClassList(mapped, localStorage.getItem("userRole"));
    ...
    if (typeof MOCK_CLASS_DATA !== 'undefined') renderClassList(MOCK_CLASS_DATA, localStorage.getItem('userRole'));
    if (typeof MOCK_PAYMENT_DATA !== 'undefined') renderClassList(MOCK_PAYMENT_DATA);
    if (typeof MOCK_COMPLAINT_DATA !== 'undefined') renderClassList(MOCK_COMPLAINT_DATA);
    if (typeof MOCK_APPROVAL_DATA !== 'undefined') renderClassList(MOCK_APPROVAL_DATA);
  }
});
