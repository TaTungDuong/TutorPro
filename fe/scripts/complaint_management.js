// Updated to call BE endpoint: /complaints

document.addEventListener('DOMContentLoaded', async function() {
  if (!requireAuthOrRedirect()) return;
  try {
    const raw = await apiFetch('/complaints');
    const items = (Array.isArray(raw) ? raw : (raw?.data || []));
    const mapped = items.map((x) => {
      
      const sender = x.parent?.name || x.parentId || '';
      return {
        id: x.id,
        sender: sender ? String(sender) : `#${x.id}`,
    ...
    if (typeof MOCK_CLASS_DATA !== 'undefined') renderComplaintTable(MOCK_CLASS_DATA, localStorage.getItem('userRole'));
    if (typeof MOCK_PAYMENT_DATA !== 'undefined') renderComplaintTable(MOCK_PAYMENT_DATA);
    if (typeof MOCK_COMPLAINT_DATA !== 'undefined') renderComplaintTable(MOCK_COMPLAINT_DATA);
    if (typeof MOCK_APPROVAL_DATA !== 'undefined') renderComplaintTable(MOCK_APPROVAL_DATA);
  }
});
